...
## نسخه v7: رفع باگ چند GRE روی یک سرور

در نسخه v7 یک باگ مهم در ساخت چند تونل GRE پشت سر هم اصلاح شد. در نسخه‌های قبلی اگر در همان اجرای اسکریپت چند تونل معمولی می‌ساختید، بعضی متغیرهای تونل قبلی، مخصوصاً `TUN_KEY`، ممکن بود داخل تونل بعدی بماند. نتیجه این بود که چند اینترفیس مثل `gre1` و `gre2` ظاهراً ساخته می‌شدند، ولی ممکن بود GRE key یا ruleها با هم قاطی شوند و بعضی تونل‌ها از کار بیفتند.

تغییرات v7:

- قبل از ساخت هر GRE، متغیرهای runtime همان بخش reset می‌شوند.
- GRE key هر تونل دوباره از شماره همان تونل ساخته می‌شود؛ مثلاً tunnel 1 => key 1 و tunnel 2 => key 2.
- اگر تونل قبلاً کانفیگ ذخیره‌شده داشته باشد، key همان کانفیگ حفظ می‌شود تا تونل‌های قدیمی خراب نشوند.
- ruleهای iptables برای هر `greN` به‌صورت جدا و در ابتدای chain اضافه می‌شوند تا ruleهای کلی مثل `DROP 10.0.0.0/8` یا UFW باعث قطع شدن تونل‌ها نشوند.
- `rp_filter` برای حالت multi-tunnel خاموش می‌شود، چون روی بعضی دیتاسنترها باعث قطع شدن GREهای متعدد می‌شود.
- گزینه جدید منو اضافه شد:

```text
5) repair/restart Normal GRE tunnel
```

با این گزینه می‌توانید فقط یک تونل GRE خاص را بدون دست زدن به تونل‌های دیگر restart/repair کنید.

برای تعمیر تونل‌های GRE موجود بعد از آپدیت به v7، روی هر دو سرور این مسیر را بروید:

```text
5) repair/restart Normal GRE tunnel
```

بعد شماره تونل را بزنید؛ مثلاً:

```text
1
```

اگر چند تونل دارید، برای هر شماره یک بار repair را بزنید.
